...
def find_subsequence(tensor, sequence):
    seq_len = len(sequence)

    # Ensure tensor is two-dimensional
    if tensor.dim() == 1:
        tensor = tensor.unsqueeze(0)

    # Create a sliding window view
    windows = tensor.unfold(1, seq_len, 1)

    # Ensure the sequence tensor is on the same device as the input tensor
    sequence_tensor = torch.tensor(sequence, device=tensor.device).unsqueeze(0).unsqueeze(0)

    # Check if each window matches the sequence
    matches = (windows == sequence_tensor).all(2)
    ...
    # Expand column indices to include each element of the sequence
    cols = cols.view(-1, 1) + torch.arange(seq_len, device=tensor.device)

    return rows.repeat_interleave(seq_len), cols.view(-1)
...
def face_vector_in_xy_to_quaternion(face_vec):
    # assert np.sum(np.abs(face_vec)) > 0
    # facing is a 3d vector representing forward direction
    face_vec = face_vec / np.linalg.norm(face_vec)
    angle = np.arctan2(face_vec[1], face_vec[0])

    # Construct quaternion
    angle_list = np.array([0, 0, angle])
    quaternion = R.from_euler('xyz', angle_list).as_quat()
    return quaternion