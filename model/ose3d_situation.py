...
from modules.layers.transformers import (TransformerEncoderLayer,
                                         TransformerSpatialEncoderLayer, CrossAttentionLayer)
from modules.layers.DiT import DiTBlock
from modules.utils import (calc_pairwise_locs, disabled_train,
                           get_mixup_function, get_mlp_head, layer_repeat,
                           maybe_autocast, transform_to_agent_coor)
...
class OSE3DSituation(BaseModel):
    def __init__(self, cfg):
        ...
        self.situation_type = self.cfg.get('situation_type', 'as_object')
        print('!!!! situation type !!!!', self.situation_type)
        if self.situation_type in ['as_object_add_loc', 'as_embedding', 'as_transform_for_objects', 'as_cross_attention', 'as_dit_attention']:
            self.loc_embedding_encoder = nn.Sequential(
                                            nn.Linear(self.cfg.loc_fourier_dim, self.cfg.hidden_size),
                                            nn.LayerNorm(self.cfg.hidden_size),
                                        )
            self.size_embedding_encoder = nn.Sequential(
                                            nn.Linear(3, self.cfg.hidden_size),
                                            nn.LayerNorm(self.cfg.hidden_size),
                                        )
        
        if self.situation_type in ['as_cross_attention']:
            situation_condition_layer = CrossAttentionLayer(
                self.cfg.hidden_size,
                nhead = self.cfg.spatial_encoder.num_attention_heads,
                dim_feedforward = self.cfg.spatial_encoder.dim_feedforward,
                dropout = self.cfg.spatial_encoder.dropout,
            ...
            self.situation_condition_layer = layer_repeat(
                situation_condition_layer,
                self.cfg.spatial_encoder.num_layers
            )

        elif self.situation_type in ['as_dit_attention']:
            situation_condition_layer = DiTBlock(hidden_size = self.cfg.hidden_size, num_heads = self.cfg.spatial_encoder.num_attention_heads)
            self.situation_condition_layer = layer_repeat(
                situation_condition_layer,
                self.cfg.spatial_encoder.num_layers
            )
    ...
    def forward(self, data_dict):
        ...
        if self.use_anchor and self.situation_type in ['as_object', 'as_object_add_loc']:
        ...
        with maybe_autocast(self, enabled=False):
            for i, pc_layer in enumerate(self.spatial_encoder):
                if self.cfg.spatial_encoder.obj_loc_encoding == 'diff_all':
                    ...
                    # 1. add location position embedding for all objects
                    if self.situation_type in ['as_object_add_loc']:
                        loc_feat = self.loc_embedding_encoder(generate_fourier_features(all_object_loc[:, :, :3]))
                        size_feat = self.size_embedding_encoder(all_object_loc[:, :, 3:])
                        query_pos = loc_feat + size_feat
                    # 2. add situation embeding for all objects
                    elif self.situation_type in ['as_embedding']:
                        loc_feat = self.loc_embedding_encoder(generate_fourier_features(all_object_loc[:, :, :3]))
                        size_feat = self.size_embedding_encoder(all_object_loc[:, :, 3:])

                        situation_loc = einops.repeat(data_dict['anchor_locs'], 'b d -> b n d', n = all_object_loc.size(1))
                        situation_loc_feat = self.loc_embedding_encoder(generate_fourier_features(situation_loc))
                        situation_ori = einops.repeat(data_dict['anchor_orientation'], 'b d -> b n d', n = all_object_loc.size(1))
                        situation_ori_feat = self.orientation_encoder(generate_fourier_features(situation_ori))
                        query_pos = loc_feat + size_feat + situation_ori_feat + situation_loc_feat
                    # 3. use distance and rotation for all objects
                    elif self.situation_type in ['as_transform_for_objects']:
                        obj_loc = all_object_loc[:, :, :3]
                        transformed_loc = transform_to_agent_coor(obj_loc, data_dict['anchor_locs'], data_dict['anchor_orientation'])
                        loc_feat = self.loc_embedding_encoder(generate_fourier_features(transformed_loc))
                        size_feat = self.size_embedding_encoder(all_object_loc[:, :, 3:])
                        query_pos = loc_feat + size_feat
                    else:
                ...
                #### situation modeling with attention ####
                if self.situation_type in ['as_cross_attention']:
                    situation_loc = einops.repeat(data_dict['anchor_locs'], 'b d -> b n d', n = all_object_loc.size(1))
                    situation_loc_feat = self.loc_embedding_encoder(generate_fourier_features(situation_loc))
                    situation_ori = einops.repeat(data_dict['anchor_orientation'], 'b d -> b n d', n = all_object_loc.size(1))
                    ...
                    obj_embeds, _ = self.situation_condition_layer[i](obj_embeds, situation_feat, tgt_key_padding_mask = all_object_mask)
                elif self.situation_type in ['as_DiT_attention']:
                    situation_loc = einops.repeat(data_dict['anchor_locs'], 'b d -> b n d', n = all_object_loc.size(1))
                    situation_loc_feat = self.loc_embedding_encoder(generate_fourier_features(situation_loc))
                    situation_ori = einops.repeat(data_dict['anchor_orientation'], 'b d -> b n d', n = all_object_loc.size(1))
