...
class SubmititLauncher:
    ...
    def __call__(self):
        ...
        self._set_gpu_args()
        if not self.args.port or not is_port_available(self.args.port, host_name):
        ...
        cmd = f"accelerate launch --num_machines {self.args.num_nodes} \
...
def submitit_launch(args):
    ...
    if args.nodelist != "":
        ...
        if len(args.nodelist) == 2:
            nodelist = f"lambda-hyperplane{str(args.nodelist)}"
        else:
...
def accelerate_launch(args):
    ...
    cfg_path, cfg_file = str(full_cfg_path.parent), str(full_cfg_path.name)
    cmd = f"accelerate launch --num_machines {args.num_nodes} \
        {multi_gpu} \
